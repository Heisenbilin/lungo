<template>
  <a-drawer v-model:visible="drawerVisible" :bodyStyle="{ padding: '0px' }" width="1000px" @close="handleCancel"
    :zIndex="999">
    <template #title>
      <div class="flex items-center">
        ...
        <div class="flex justify-between w-3/5">
          <a-button type="link" target="_blank" href="https://app.xesv5.com/doc/pages/fedata/monitor.html">
            操作文档
          </a-button>
          <a-popconfirm title="关闭项目后，将保存此表单信息并中止日志解析。确定要关闭吗？" ok-text="是" cancel-text="否" @confirm="handleOk(true)">
    ...
    <a-form :style="{ marginBottom: '60px' }" v-else ref="formRef" :model="formState" :rules="rules"
      ...
      <a-collapse v-model:activeKey="activeKey" expandIconPosition="right">
        <template #expandIcon="{ panelKey, isActive }">
          <a-switch :checked-children="panelKey !== '4' ? '开' : '是'" :un-checked-children="panelKey !== '4' ? '关' : '否'"
            :checked="isActive" v-if="panelKey !== '1'" />
        </template>
          ...
          <a-row>
            ...
            <a-col :span="12">
              <a-form-item label="应用类型" :name="['basic_info', 'saas']">
                <a-select v-model:value="formState.basic_info.saas" placeholder="素质/学科（saas项目）">
            ...
            <a-col :span="12">
              <a-form-item :name="['basic_info', 'appid']">
                ...
                <a-input v-model:value="formState.basic_info.appid" placeholder="请输入日志中心申请到的appid" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item label="eventid" :name="['basic_info', 'eventid']">
                <a-input v-model:value="formState.basic_info.eventid" placeholder="必须与应用内的eventId配置完全一致" />
              </a-form-item>
            </a-col>
            <a-col :span="24">
              <a-form-item :name="['basic_info', 'uc_group_id']" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
                <template #label>
                  关联用户组
                  <a @click="openUCGroup">
                    <FormOutlined />
                ...
                <a-select v-model:value="formState.basic_info.uc_group_id" allowClear showSearch placeholder="请选择用户组"
                  :filterOption="filterOption" @change="handleChangeUcGroup">
                  <a-select-option v-for="item of groups" :value="item.group_id" :key="item.group_id">
                    {{ item.group_name }}
                  </a-select-option>
                </a-select>
              </a-form-item>
              <a-form-item :name="['basic_info', 'gateway']" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
                <template #label>
                  所属网关
                ...
                <a-select v-model:value="formState.basic_info.gateway" allowClear showSearch placeholder="请选择所属网关"
            ...
            <a-col span="18">
              <a-form-item :name="['basic_info', 'domain']" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
                <template #label>
                  网关域名
                ...
                <a-input v-model:value="formState.basic_info.domain" placeholder="请输入项目页面在网关的域名(upstream名称)，用于筛选网关日志" />
              </a-form-item>
            </a-col>
            <a-col span="6">
            ...
            <a-col :span="12">
              <a-form-item label="所属项目" :name="['basic_info', 'belong_project']">
                <a-input v-model:value="formState.basic_info.belong_project" placeholder="例如：鲲系统" />
              </a-form-item>
            </a-col>
            <a-col :span="24">
              <a-form-item :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
                ...
                <a-input v-model:value="formState.basic_info.href"
                  placeholder="请输入日志解析需要筛选的URL关键字，多个关键字用英文逗号(,)隔开。无需筛选请不要填写此项！" />
              </a-form-item>
              <a-form-item label="项目描述" :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
                <a-textarea v-model:value="formState.basic_info.project_desc" placeholder="最多250个字" />
              </a-form-item>
              <a-form-item :label-col="{ span: 3 }" :wrapper-col="{ span: 21 }">
                <template #label>
                  接收周报用户
                  <a-tooltip :overlayStyle="{ maxWidth: '550px' }" title="选中的用户在每周一上午，从知音楼企业助手上收到项目本周的质量周报">
                    <InfoCircleOutlined />
                  </a-tooltip>
                </template>
                <a-alert v-if="showAlert" type="warning" show-icon>
                  <template #message>
                    <div class="flex items-center justify-between">
                      ...
                      <CloseOutlined :style="{ fontSize: '12px' }" class="cursor-pointer"
                        @click="() => (showAlert = false)" />
                    </div>
                  </template>
                </a-alert>
                <GroupUsersCheckbox :base-url="baseURL" popover :uc-group-id="formState.basic_info.uc_group_id"
        ...
        <a-collapse-panel key="2">
          <template #header>
            <a-row>
              ...
              <a-col :span="6">
                <a-tooltip :overlayStyle="{ maxWidth: '460px' }"
          ...
          <a-row>
            <a-col :span="8">
              <a-form-item :label-col="{ span: 12 }" :wrapper-col="{ span: 12 }">
                <template #label>
                  使用私有云存储
                  <a-tooltip :overlayStyle="{ maxWidth: '340px' }"
          ...
          <a-row>
            <a-col :span="12">
              <a-form-item label="上传插件是否打印日志" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
          ...
          <a-row>
            ...
            <a-col :span="16" v-if="formState.sourcemap_info.default_upload === 1">
              <a-form-item>
                ...
                <a-input placeholder="请输入map文件所在文件夹相对位置，多个文件夹请用英文逗号 (,) 区分"
                  v-model:value="formState.sourcemap_info.upload_from" />
              </a-form-item>
              <a-form-item label="map文件上传位置">
                <a-input placeholder="上传到存储桶中的对应文件夹位置（可不填）" v-model:value="formState.sourcemap_info.upload_to" />
              </a-form-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="8">
              <a-form-item label="知音楼消息提醒" :label-col="{ span: 12 }" :wrapper-col="{ span: 12 }">
        ...
        <a-collapse-panel key="3" header="自定义UA解析">
          ...
          <template v-for="(item, index) in formState.ua_info" :key="index">
            <a-card :title="'UA信息' + (index + 1)" style="width: 100%">
              <template #extra>
                <a-button v-if="index > 0" danger type="primary"
                  class="w-20 h-5 flex justify-center items-center !text-xl !p-0" @click="addOrDelUA(index)">
                  -
                </a-button>
                <a-button v-else class="w-20 h-5 flex justify-center items-center !text-xl !p-0" type="primary"
              ...
              <a-row align="middle">
                <a-col :span="10">
                  <a-form-item :name="['ua_info', index, 'ua_name']" label="UA字段" :rules="rules.ua_info.ua_name">
                    <a-input v-model:value="item.ua_name" @input="hideParse(index)" />
                  </a-form-item>
                  <a-form-item :name="['ua_info', index, 'ua_flag']" label="关键字" :rules="rules.ua_info.ua_flag">
        ...
        <a-collapse-panel key="4">
          <template #header>
            <a-row>
              <a-col :span="17">
                登录认证参数
                <a-tooltip :overlayStyle="{ maxWidth: '340px' }" title="为了准确生成每周的质量周报，需要登录的项目请在此处设置登录验证参数">
                  <InfoCircleOutlined />
                </a-tooltip>
              </a-col>
          ...
          <div v-if="formState.auth_info.need_login === 1">
            <a-row>
              <a-col :span="24">
                <a-form-item labelAlign="left" label="项目url" :name="['auth_info', 'project_url']"
                  :rules="rules.authRules.project_url" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }">
                  <a-input placeholder="请输入项目url，需包含完整的域名和协议如：https://fedata.xesv5.com"
            ...
            <a-row>
              <a-col :span="24">
                <a-form-item labelAlign="left" label="登录参数有效期" :name="['auth_info', 'effective_time']"
                  :rules="rules.authRules.effective_time" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
                  <a-radio-group v-model:value="formState.auth_info.effective_time" name="radioGroup">
            ...
            <a-row>
              <a-col>
                <a-form-item labelAlign="left" :label="noticeSlot" :name="['auth_info', 'need_notify']"
                  :rules="rules.authRules.need_notify" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
            ...
            <a-row v-if="formState.auth_info.need_notify === 1">
              <a-col :span="24">
                <a-form-item label="知音楼提醒用户" labelAlign="left" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }"
                  :name="['auth_info', 'notice_users']" :rules="rules.authRules.notice_users">
                  <a-alert v-if="showAlert" type="warning" show-icon>
                    <template #message>
                      <div class="flex items-center justify-between">
            ...
            <a-row type="flex">
              <a-col :span="5">
                <div>
                  <span>认证参数</span>
                  <a-tooltip :overlayStyle="{ maxWidth: '340px' }" title="该登录参数可用于放火平台lighthouse">
                    <InfoCircleOutlined />
                  </a-tooltip>
                </div>
              </a-col>
            ...
            <div v-for="(item, i) in formState.auth_info.authForm.fields" :key="item.index" class="auth-form-item">
              <a-row align="top">
                <a-col :span="3" />
                <a-col :span="1" class="mt-2">
                  <i v-if="i === 0" @click="addAuthField(formState.auth_info.authForm)" class="mt-2 cursor-pointer">
                    <PlusCircleTwoTone />
                  </i>
                </a-col>
                <a-col :span="5">
                  <a-form-item :name="['auth_info', 'authForm', 'fields', i, 'field']" :rules="rules.authRules.field">
                ...
                <a-col :span="5">
                  <a-form-item :name="['auth_info', 'authForm', 'fields', i, 'key']" :rules="rules.authRules.key">
                    <a-input placeholder="请输入KEY" v-model:value="item.key"
                      @change="() => clearFieldsSpace(formState.auth_info.authForm, i, 'key')" />
                  </a-form-item>
                </a-col>
                <a-col :span="8">
                  <a-form-item :name="['auth_info', 'authForm', 'fields', i, 'value']" :rules="rules.authRules.value">
                    <a-textarea v-model:value="item.value" placeholder="请输入VALUE" auto-size clearable />
                  </a-form-item>
                </a-col>
                <a-col :span="1" class="mt-2">
                  <i v-if="i !== 0" class="cursor-pointer"
          ...
          <a-row type="flex" justify="space-between" align="middle">
            ...
            <a-col :span="5">
              <a-button type="primary" :loading="formState.auth_info.checkLoading" @click="
                checkAuth(
                  formRef,
                  formState.auth_info,
    ...
    <div class="absolute right-0 bottom-0 w-full border-t-gray-300 border-t-1 px-2 py-3 bg-white text-right z-1">
