...
//图表基础配置
const performanceChartConfig: any = {
  tooltip: {
    trigger: 'axis',
    axisPointer: {
      type: 'shadow',
...
const handleDataToChartData = orginData => {
  ...
  const nameList: any[] = [] //seris名称
  const countList: any[] = [] //计数数据
  const percentArrList: any[] = [] //百分比数据
  ...
  const dataList: any = nameList.map((name, i) => ({
    name: name,
    type: 'bar',
    stack: 'total',
    barMaxWidth: 40,
