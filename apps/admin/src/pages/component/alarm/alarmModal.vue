<template>
  <div>
    <a-modal v-model:visible="visible" title="告警设置" :footer="null" width="80%">
      <a-button type="primary" @click="addAlarmRule"> 新增规则</a-button>
      <div></div>
...
import { alarmActionLogColumns, alarmLogColumns, alarmRuleColumns } from './alarmTableConfig'
...
const store =
  props.page === 'board' ? boardStore : props.page === 'panel' ? panelStore : reportStore

const { alarmModalVisible: visible } = storeToRefs(store)
watch(visible, () => {
  if (visible.value) {
    getAlarmRuleList()
  }
...
const alarmFormVisible = ref(false)
// 打开修改表单
const openUpdateForm = record => {
  recordData.value = record
  alarmFormVisible.value = true
}
