  ...
  <div>
    ...
    >
      <a-form ref="formRef" :model="alarmForm" v-bind="formItemLayout" :rules="rules">
        ...
        <a-row>
          <a-col :span="10">
            <a-form-item label="监控周期" v-bind="defaultItemLayout" name="interval">
              <a-input-number
                v-model:value="alarmForm.interval"
                :min="10"
                :step="10"
                :max="60"
        ...
        <a-row>
          <a-col :span="10">
            <a-form-item label="升级周期" v-bind="defaultItemLayout" name="upgradeCondition">
              <a-input-number
                v-model:value="alarmForm.upgradeCondition"
                :min="10"
                :step="10"
                :max="60"
                :formatter="value => `${value}分钟`"
        ...
        <a-alert
          message="告警群设置需要借助知音楼群中添加的机器人的WebHook与密钥来推送告警"
          type="warning"
          closable
        />
        <a-form-item label="告警群设置">
          <div v-for="(item, index) in settingObj.robot" :key="index">
            <a-row :gutter="16">
              <a-col :span="10"
                ><a-input v-model:value="item.yachUrl" placeholder="WebHook"
              /></a-col>
              <a-col :span="10">
        ...
        <a-form-item :wrapper-col="{ span: 14, offset: 8 }">
          <a-button type="primary" @click="onUpdateSubmit" v-if="updateAlarmRuleId > 0"
            >修改</a-button
          >
...
const rules = {
  ...
  upgradeCondition: [{ required: true, type: 'number', message: '升级周期必填', trigger: 'blur' }],
