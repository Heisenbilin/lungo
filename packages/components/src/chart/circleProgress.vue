<template>
  <div class="circle-main">
    <svg
      style="transform: rotate(-90deg)"
      :width="width"
      :height="width"
      xmlns="http://www.w3.org/2000/svg"
    >
      <circle
        :r="(width - radius) / 2"
        :cy="width / 2"
        :cx="width / 2"
        :stroke-width="radius"
        :stroke="backgroundFinColor"
        :fill="backgroundFinColor"
      />
      <circle
        ref="$bar"
        :r="(width - radius) / 2"
        :cy="width / 2"
        :cx="width / 2"
        :stroke="barFinColor"
        :stroke-width="radius"
        :stroke-linecap="isRound ? 'round' : 'square'"
        :stroke-dasharray="(width - radius) * 3.14"
        :stroke-dashoffset="
          isAnimation ? (width - radius) * 3.14 : ((width - radius) * 3.14 * (100 - progress)) / 100
        "
        :fill="backgroundFinColor"
      />
    </svg>
    <div class="score" :style="{ color: barFinColor }">{{ progress }}{{ hasUnit ? '分' : '' }}</div>
  </div>
</template>

<script>
export default {
  props: {
    width: {
      type: [Number, String],
      default: 160,
    }, // 圆的大小
    radius: {
      type: [Number, String],
      default: 15,
    }, // 进度条厚度
    progress: {
      type: [Number, String],
      default: 0,
    }, // 进度条百分比
    hasUnit: {
      type: Boolean,
      default: true,
    }, //是否有“分”这个单位
    // barColor: {
    //   type: [String],
    //   default: '#F2AE57'
    // }, // 进度条颜色
    // backgroundColor: {
    //   type: [String],
    //   default: '#FFE8CC'
    // }, // 背景颜色
    isAnimation: {
      // 是否是动画效果
      type: Boolean,
      default: false,
    },
    isRound: {
      // 是否是圆形画笔
      type: Boolean,
      default: true,
    },
    id: {
      // 组件的id，多组件共存时使用
      type: [String, Number],
      default: 1,
    },
    duration: {
      // 整个动画时长
      type: [String, Number],
      default: 1000,
    },
    delay: {
      // 延迟多久执行
      type: [String, Number],
      default: 20,
    },
    timeFunction: {
      // 动画缓动函数
      type: String,
      default: 'cubic-bezier(0.99, 0.01, 0.22, 0.94)',
    },
  },
  data() {
    return {
      idStr: `circle_progress_keyframes_${this.id}`,
    };
  },
  computed: {
    barFinColor() {
      if (this.progress < 50) {
        return '#ec5c4c';
      }
      if (49 < this.progress && this.progress < 75) {
        return '#F2AE57';
      }
      if (74 < this.progress) {
        return '#5eca75';
      }
    },
    backgroundFinColor() {
      if (this.progress < 50) {
        return '#f9cec9';
      }
      if (49 < this.progress && this.progress < 75) {
        return '#fbe5c4';
      }
      if (74 < this.progress) {
        return '#cfefd6';
      }
    },
  },
  beforeUnmount() {
    // 清除旧组件的样式标签
    document.getElementById(this.idStr) && document.getElementById(this.idStr).remove();
  },
  mounted() {
    if (this.isAnimation) {
      // 重复定义判断
      if (document.getElementById(this.idStr)) {
        console.warn('vue-circle-progress should not have same id style');
        document.getElementById(this.idStr).remove();
      }
      // 生成动画样式文件
      let style = document.createElement('style');
      style.id = this.idStr;
      style.type = 'text/css';
      style.innerHTML = `
      @keyframes circle_progress_keyframes_name_${this.id} {
      from {stroke-dashoffset: ${(this.width - this.radius) * 3.14}px;}
      to {stroke-dashoffset: ${
        ((this.width - this.radius) * 3.14 * (100 - this.progress)) / 100
      }px;}}
      .circle_progress_bar${this.id} {animation: circle_progress_keyframes_name_${this.id} ${
        this.duration
      }ms ${this.delay}ms ${this.timeFunction} forwards;}`;
      // 添加新样式文件
      document.getElementsByTagName('head')[0].appendChild(style);
      // 往svg元素中添加动画class
      this.$refs.$bar.classList.add(`circle_progress_bar${this.id}`);
    }
  },
};
</script>

<style scoped lang="scss">
.circle-main {
  position: relative;
  text-align: -webkit-center;
}
.score {
  position: absolute;
  transform: translate(-50%, -50%);
  top: 50%;
  left: 50%;
  font-size: 140%;
  font-weight: bold;
}
</style>
